...
class ResNet(nn.Module):
    ...
    def forward(self, x):
        outputs = []
        # np.save(os.path.join('/workspace/retinanet/debug', 'x.npy'), x.cpu().numpy())
        x = self.stem(x)
        # np.save(os.path.join('/workspace/retinanet/debug', 'stem.npy'), x.cpu().numpy())
        for stage_name in self.stages:
            x = getattr(self, stage_name)(x)
            if self.return_features[stage_name]:
                outputs.append(x)
                # np.save(os.path.join('/workspace/retinanet/debug', '{}.npy'.format(stage_name)), x.cpu().numpy())
        return outputs
...
class StemWithFixedBatchNorm(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.conv1(x)
        # np.save(os.path.join('/workspace/retinanet/debug', 'stem_conv.npy'), x.cpu().numpy())
        # np.save(os.path.join('/workspace/retinanet/debug', 'stem_conv_weight.npy'), self.conv1.weight.cpu().numpy())
        x = self.bn1(x)
        # np.save(os.path.join('/workspace/retinanet/debug', 'stem_bn.npy'), x.cpu().numpy())
        x = F.relu_(x)
        # np.save(os.path.join('/workspace/retinanet/debug', 'stem_relu.npy'), x.cpu().numpy())
        x = F.max_pool2d(x, kernel_size=3, stride=2, padding=1)
        # np.save(os.path.join('/workspace/retinanet/debug', 'stem_pool.npy'), x.cpu().numpy())
        return x
